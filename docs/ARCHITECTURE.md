...
- `session.freshAge = 0` — otherwise Better Auth's `list-sessions` 403s (`SESSION_NOT_FRESH`) after a day and breaks the Settings sessions card. Freshness is re-imposed (`middleware/fresh-session.ts`) only where Better Auth itself had it before `freshAge: 0` removed it: `unlink-account`, and `delete-user` (Better Auth checks it there when no password is given). Not on `update-user` (name and image only; Better Auth refuses an email there) nor on the three `revoke-*session*` routes (Better Auth only re-reads the session from D1 there, `sensitiveSessionMiddleware`): gating those made a rename and "Sign out other devices" fail with 403 for anyone signed in over a day. The gate's 403 carries `message`, which the auth client shows; with only `error` the screen said "Failed to …".
- **Accounts are deactivated, never deleted.** Deleting a `user` row cascades: `time_entries.user_id` and `tasks.created_by` go NULL (reports lose the author), `recurring_entries`, `task_assignees`, `integrations` and `assistant_memory` rows go, and a legacy `workspaces.userId` pointing at the person takes the whole workspace with it. So `POST /api/auth/delete-user` is served by `routes/account.ts` (Better Auth's own `deleteUser` is disabled), `/api/auth/admin/remove-user` answers 403, and the admin `DELETE /api/admin/users/:id` deactivates too. Deactivation (`lib/account-deactivation.ts`) bans the user with `banReason` "Account deactivated", deletes their sessions and `member` rows, pauses their recurring templates and calendar auto-track, and turns off their digests, all in one batch; it refuses the only owner of a workspace (409 `LAST_OWNER`). Being invited again (`sendInvitationEmail`) lifts that ban, never a moderation ban with another reason.
...
- **Email** goes out through the Resend API (`lib/mailer.ts`), from the address in the `EMAIL_FROM` var: invites, OTP codes, magic links, digests. Not the `send_email` binding, which needs Email Routing on the zone and would move the domain's MX records off the company's real mail; Resend only needs the domain verified for sending. **Only a deployed app sends real mail**: `shouldDeliver` refuses when `APP_URL` is localhost and refuses RFC 2606 test domains (`.test`, `.example`, `example.com`…) everywhere, logging `[mailer] not sent` with the body instead, so local dev and the e2e suite (which reuses the running `pnpm dev`) can exercise every email flow without delivering anything. Bodies are React Email templates (`src/worker/emails/*.tsx`) rendered with `render`/`toPlainText` from `react-email`; `pnpm email:dev` serves a local template preview.
...
- **66 tools (26 read, 40 write)** from one catalog (`mcp/registry.ts`) that also feeds the in-app Assistant. Most run through `mcp/rest-bridge.ts`, the app's own routers with the key's workspace and person, so a tool gets the screen's validation, role checks and broadcasts. A key acts as the person who created it: a member's key reads only their own hours and no budgets, and `log_time`/`create_project` require an active project/client. Timers are app-only (no start/stop tools). `list_task_comments` returns the newest 100 and takes `limit`/`before` to read further back.
- **Writes go through the app's routes, never around them.** Every write tool calls the same route the screen does, via the bridge — `move_task` is the board's `PATCH /api/tasks/:id/move`, `create_task` is `POST /api/tasks` — so the subtask cascade, recurrence spawn, column order, task history, notifications and live broadcast can't diverge between the app and the MCP (a private `moveTaskStatus` once skipped all five). `create_client`/`create_project`/`draft_day` call the same function their route calls.
- **Lists, not twins.** `create_task`, `move_task`, `update_task`, `delete_task`, `log_time`, `update_time_entry` and `delete_time_entry` take one item or `items` (`mcp/batch.ts`: `listableInput` + `runListable`), so several items are one call and one approval, with a per-item report; `delete_time_entry` sends a list to the all-or-nothing `/bulk` route. The Assistant adds "Approve all" when one answer asks for several actions.
- **Results are cheap to read.** JSON is unindented, `list_tasks` returns a lean row per task in the board's column order (notes come from `get_task`), and `get_task` reads one task (`GET /api/tasks/:id`) plus its subtasks (`?parentId=`).
...
A task description can be edited by several people at once when `COLLAB_DESCRIPTIONS` is `"true"` (only set in `.dev.vars` today; unset in the deploy vars, `/api/me` reports `features.collabDescriptions: false` and no socket is opened).

- **Transport**: `GET /api/collab/descriptions/:taskId` (session + workspace middleware, then the task must belong to the caller's workspace — the same rule as `PUT /api/tasks/:id`) upgrades to the `DescriptionRoom` Durable Object named `<workspaceId>:<taskId>` (`y-partyserver`'s `YServer`, Yjs sync + awareness).

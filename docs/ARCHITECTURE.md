...
- `session.freshAge = 0` — otherwise Better Auth's `list-sessions` 403s (`SESSION_NOT_FRESH`) after a day and breaks the Settings sessions card. Freshness is re-imposed selectively on sensitive ops (`unlink-account`, `delete-user`, the `revoke-*session*` routes). Not on `update-user`: it can only change name and image (Better Auth refuses an email there), Better Auth never gated it, and gating it made renaming fail with 403 for anyone signed in over a day.

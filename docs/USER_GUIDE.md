...
**Moving and reshaping lines.** Hover any line and a **⠿** handle appears to its left (on an empty line, a **+** too). **+** opens the **/** menu on that line. **⠿** does two things: grab it to drag the line somewhere else (a red line shows where it will land), or click it for **Turn into**, **Color**, **Duplicate** and **Delete**. A long description shows its first lines faded out with an **Expand** button; clicking into it expands it too.

...
**Opening a task: modal or sidebar.** Clicking a task opens it in a **modal** (the default): the task's fields, notes, subtasks and attachments on the left, and its comments always visible on the right — no tab to switch. Hover (or click) the view button at the top right of the task and pick **Modal** or **Sidebar** (the side panel, with Task and Comments as tabs). The task's fields sit two per line in the modal and one per line in the sidebar. Whichever you pick is remembered in this browser; another browser starts on the modal. On a narrow screen the modal puts the comments below the task.

**Formatting notes.** Select text in a task's description and a formatting bar appears over it: text or heading 1–3, bold, italic, underline, strikethrough, inline code, text color, link, bulleted/numbered list and checklist. Type **/** at the start of a line for a menu of block types (text, headings, quote, lists, checklist) and pick one with the arrows and Enter; typing after the slash filters it (`/check`, `/titulo`). Typing shortcuts still work (`# ` for a heading, `**bold**`, `[] ` for a checklist). The menu also has **code block** and **divider** (`/codigo`, `/divisor`).

**Moving and reshaping lines.** While you edit, an empty line shows a short hint ("Write, or type / for commands"). Hover a line with text and a **⠿** handle appears to its left; hover an empty line and a **+** appears instead, which opens the **/** menu on that line. **⠿** does two things: grab it to drag the line somewhere else (a red line shows where it will land), or click it for **Turn into**, **Color**, **Duplicate** and **Delete**. A long description shows its first lines faded out with an **Expand** button; clicking into it expands it too.
...
**Statuses and the board.** Every task sits in a status — a workspace starts with seven: **Backlog → On hold / Stuck → To do → In progress → QA → Client review → Closed** (new tasks land in To do; Closed is the completed one) — and the **Board** tab shows one column per status. Drag a card between columns, or reorder within one; a drop saves straight away and appears on your teammates' boards within a few seconds. The project rail on the left filters the board the same way it filters every other tab. The chip on each row in the list shows the same status, and clicking it changes it without opening anything.

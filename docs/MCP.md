...
|---|---|---|
| Time | `get_running_timer`, `list_time_entries`, `get_time_entry`, `get_time_summary`, `run_report`, `list_drafts` | `log_time`, `log_times`, `update_time_entry`, `update_time_entries`, `delete_time_entry`, `delete_time_entries`, `copy_week`, `draft_day` |
...
**Batch tools** take `items` (up to 50; each item is what the single tool takes) or, for entries, `entryIds`: one call, so one approval in the client, with a per-item report (`done`, `failed`, `results`). Tasks: `create_tasks`, `move_tasks`, `update_tasks`, `delete_tasks`; entries: `log_times`, `update_time_entries`, `delete_time_entries` (the last two all-or-nothing). `list_tasks` returns tasks grouped by status in the board's column order, completed ones only with `includeDone`.

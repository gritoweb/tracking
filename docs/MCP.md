...
**Several at once:** `create_task`, `move_task`, `update_task`, `delete_task`, `log_time`, `update_time_entry` and `delete_time_entry` take one item as always, or `items` — a list of the same fields (up to 50; 200 for `delete_time_entry`) — in one call, so one approval in the client, with a per-item report (`done`, `failed`, `results`). `delete_time_entry` with `items` is all or nothing (the entries' bulk route). `list_tasks` returns tasks grouped by status in the board's column order, completed ones only with `includeDone`.

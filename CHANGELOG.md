...
### Fixed
- **Deleting an open subtask** reads "Delete subtask" in the ⋯ menu and "Delete subtask?" in the confirmation, keeps the subtask open until you confirm, and then lands on its parent. It already deleted only the subtask (the server removes the row plus its children, and a subtask has none); what was wrong was the path afterwards: the list refetch rendered before the router's navigation (v7 runs it as a transition) with the deleted id still in the route, so the "task doesn't exist" guard toasted and sent you to `/tasks`. `TaskBoardList` now remembers ids deleted from that screen, and the guard skips them.
### Verified
- `tsc -b` (0), `lint` (0), `vitest run` 1096/1096 (new `TaskSubtasks` menu test; menus from separate renders don't reopen under jsdom, so the menu is opened once per file). e2e `subtask-delete` 2/2 (row menu deletes the subtask and keeps the parent; open-subtask delete lands on the parent with its panel open, and the API confirms the subtask is gone and the parent isn't). Before the fix the second one ended on `/tasks`, and instrumenting the guard showed it firing with the deleted id while the URL was already the parent's. `task-delete-permissions` + `task-detail-panel` 10/10. Mail stays off: `APP_URL` is localhost and e2e addresses are `@example.com`, so `shouldDeliver` returns before any request to Resend.

## 2026-09-25 (112) — on `refactor`, not deployed
### Added
- **Unread notifications reach people on Slack.** A bell notification (task assigned, @mention, status change) that stays unread for 15 minutes is sent to its person as a Slack DM from the TimeTracker bot, with an "Open in TimeTracker" button; several pending ones become one grouped DM. Why: people who don't keep the app open never saw them. One installation per workspace through OAuth ("Add to Slack" in Settings → Workspace → Integrations, owner/admin only); people are matched by email (`users.lookupByEmail`, cached in `slack_user_links`); each person can opt out. Only notifications from the last 24 hours go out, so installing doesn't replay history. At most once: the cron claims each row (`notifications.slack_sent_at`) before sending. A revoked token removes the installation. Slack text is escaped, so a task name can't become `<!channel>` or a fake link.
- New: migration `0053_slack_notifications.sql` (additive only), `lib/slack.ts` (`runSlackNotifications` in the 5-minute cron), `routes/slack.ts` (`/api/slack`), `SlackSection` inside the Integrations card, `hooks/useSlack.ts`, `docs/SLACK.md` (app manifest + setup). Env: `SLACK_CLIENT_ID`, `SLACK_CLIENT_SECRET`, and `SLACK_DRY_RUN=1` for local dev, which logs every Slack call instead of making it.
- Slack has its own table rather than a row in `integrations`: that table's contract is pushing time (`IntegrationAdapter.pushTimeEntry`, assigned per project), and Slack is a notification channel.
### Verified
- `tsc -b` (0), `lint` (0), `pnpm check` (0), `vitest run` 1090/1090 at the time (new: 11 sweep tests on a real migrated SQLite with `fetch` mocked, 6 route tests, 4 `SlackSection` render tests). Mutation check: loosening the member join failed the ex-member test. Local end to end with `SLACK_DRY_RUN=1`: migration applied `--local` only; with a fake installation, firing the cron logged the lookup and the DM payload, set `slack_sent_at` on the 20-minute-old notification and left the 2-minute-old one alone. Zero requests to slack.com, zero `[mailer]` lines, zero Resend calls. Test rows deleted afterwards. Deliberately not done: no real Slack app, no real message, no remote migration, no deploy.

## 2026-09-25 (112) — on `refactor`, not deployed
### Added
- **Unread notifications reach people on Slack.** A bell notification (task assigned, @mention, status change) still unread after 15 minutes is sent to its person as a Slack DM from the TimeTracker bot, with an "Open in TimeTracker" button; several pending ones become one grouped DM. Why: people who don't keep the app open never saw them. One installation per workspace through OAuth ("Add to Slack" in Settings → Workspace → Integrations, owner/admin only); the person is matched by email (`users.lookupByEmail`, cached in `slack_user_links`); each person can opt out. Only notifications from the last 24 hours, so installing doesn't replay history. At most once: the cron claims each row (`notifications.slack_sent_at`) before sending. A revoked token removes the installation. Slack text is escaped so a task name can't become `<!channel>` or a fake link.
- New: migration `0053_slack_notifications.sql` (additive only), `lib/slack.ts` (`runSlackNotifications` in the 5-minute cron), `routes/slack.ts` (`/api/slack`: status, opt-out, connect/callback, disconnect, test), `SlackSection` inside the Integrations card, `hooks/useSlack.ts`, `docs/SLACK.md` (Slack app manifest + setup). Env: `SLACK_CLIENT_ID`, `SLACK_CLIENT_SECRET`, and `SLACK_DRY_RUN=1` for local dev, which logs every Slack call instead of making it.
- Slack lives in its own table rather than `integrations`: that table's contract is pushing time (`IntegrationAdapter.pushTimeEntry`, per-project assignment), and Slack is a notification channel.
### Verified
- `tsc -b` (0), `lint` (0), `vitest run` (new: 11 sweep tests on a real migrated SQLite with `fetch` mocked — delay window, read/fresh/stale skipped, no re-send, opt-out, ex-member, email not found cached, revoked token, dry run makes no fetch, escaping, backlog cap; 6 route tests — manager-only install, encrypted token after callback, state from another workspace or forged refused, member can't disconnect, opt-out reflected in status; 4 `SlackSection` render tests). Mutation check: loosening the member join made the ex-member test fail. Local end to end with `SLACK_DRY_RUN=1`: migration applied `--local` only; with a fake installation, firing the cron logged the lookup and the DM payload, set `slack_sent_at` on the 20-minute-old notification and left the 2-minute-old one alone; zero requests to slack.com, zero `[mailer]` lines, zero Resend calls; test rows deleted afterwards. Not done on purpose: no real Slack app, no real message, no remote migration, no deploy.

## 2026-09-24 (111) — on `refactor`, not deployed
### Changed
- **Comments use the description's editor.** Composing and editing a comment is `CommentComposer` (the same `RichTextEditor` at `density="compact"`, with the new reusable `EditorToolbar`: "+" for the block menu, a clip for an image, "@" to tag someone, Ctrl/⌘+Enter to send), so headings, lists, checklists, code, quotes, formatting and inline images all work in a comment; a comment is shown with the same editor read-only. Old comments (text with `@[Name](user:ID)` tags) open and show unchanged through `commentDoc`.
- **Stored format**: a new comment body is the same tiptap JSON doc as a description. `src/shared/comment-body.ts` (`commentText`) reads either kind back as the old text form, so mention lookup, the mention notification's text and MCP's `list_task_comments` read both identically; MCP and the Assistant can still write plain text. `docToText` in `src/shared/rich-doc.ts` is now the one depth-capped reader of a doc as text (MCP's `richTextToPlain` reuses it). Body cap raised to the description's `TASK_DESCRIPTION_MAX`; a doc with no text, mention or image is refused (400) like empty text. No migration.
- **Comment cards**: `Card` gains `size="compact"` and `look="outlined"` (hairline, `rounded-lg`, the surface's own background, border in the focus colour while a field inside has focus); editing happens in the same card under the same header, so nothing moves. `cn()` now knows `rounded-container`, so a variant can replace it.
- **Pieces extracted for reuse**: `editorUpload.ts` (image upload into any editor), `useTaskImageUpload` (checked upload + orphan cleanup, shared by description and comments), `useEscapeLocal` + `keepOpenOnLocalEscape` in `DialogContent`/`SheetContent`.
### Fixed
- **Esc while editing a comment closed the whole task** (Radix handles Esc before the field), losing the edit. Esc now cancels the edit and keeps the task open; with a "/" or "@" menu open, Esc closes only the menu (Radix hands Esc to the topmost layer).
### Verified
- `tsc -b` (0), `lint` (0), `vitest run` 1073/1073 (new: `comment-body`, rich-body mention and empty-doc route tests, `Card` looks, `cn` radius). e2e `task-comments` 4/4 after moving its selectors to the editor's accessible names (and `exact` on the "Comment" button, which "Hide comments" also matched); the Resend log shows no new send. In the browser: "+" opened the block menu (heading made), "@" the people list, Ctrl+Enter posted; saved as a doc, `mentionedUserIds` filled from it, Co Editor's notification text readable; old comment still shown; edit opens the doc in place; Esc with a menu open closes only the menu, plain Esc cancels without closing the task.
...
### Fixed
- **Tests sent real email.** The e2e suite reuses the running `pnpm dev`, whose `.dev.vars` holds a real Resend key, and every invite in it (`invitee-…@outsider.test`) went out — 100+ today, exhausting the daily quota. `shouldDeliver` in `lib/mailer.ts` now refuses a localhost `APP_URL` and RFC 2606 test domains everywhere, logging `[mailer] not sent` with the body instead. The sender moved from code to the `EMAIL_FROM` var.
- **The "+" handle's "/" menu closed as soon as it opened**: tiptap applies `focus()` a frame later, and the popover read that as focus leaving it. The suggestion list now closes only on the plugin's own exit.
- **Dark mode description**: the unchecked checklist box measured ~1.3:1 (`--border` is 9% white); it and the quote bar now derive from `--muted-foreground`.
- **Subtask field invisible until clicked**: the bare quick-add now has the Input's own border, fill and focus ring; the last subtask row no longer draws a divider onto it.
### Verified
- `tsc -b` (0), `lint` (0), `vitest run` 1055/1055 (new: block actions on a real editor, `Expandable`, `shouldDeliver`), `pnpm check` (0, dry-run only). In the browser: handle, "+", drag (block moved, drop line shown, order saved), block menu, divider, dark mode. Two people (two isolated sessions) on one description: each saw the other's typing before any save, carets with names, merged content saved once, no duplication after a remount. An invite to `@outsider.test` logged `[mailer] not sent` and the Resend list did not grow.
...
- **The Assistant said "Done!" about a move that never happened.** Luis approved a `move_task`; the task stayed in Backlog, and on his next message the model reported success. Evidence: the conversation stored in the ChatAgent's local Durable Object had that part in `approval-responded` with no output, the task's history had one move (the later retry), and the dev log had `AI_InvalidToolApprovalSignatureError … missing signature` for exactly that approval id — three times today. Cause: `ChatAgent` signs every approval (`experimental_toolApprovalSecret`, the S-02 security fix), and the Agents SDK (`agents@0.17.4`, and 0.24.0 too) drops the signature when it persists the approval request (`message-builder`'s `applyChunkToParts` keeps only `id`/`descriptor`). Run from the browser's copy (which has it) an approval works; run from the stored copy it fails, the action never runs, and the chat showed nothing.
  - `patches/agents@0.17.4.patch` (`pnpm patch`, registered under `pnpm.patchedDependencies`) copies `chunk.signature` onto the stored approval. The signing stays on. Shown side by side on the same chunk: unpatched `{"id":"approval-1"}`, patched `{"id":"approval-1","signature":…}`. `approval-signature.test.ts` fails if an upgrade drops it.
  - A tool card approved but without a result once the turn is over now reads "… was approved but didn't run — ask again to retry" instead of looking done; a failed turn shows an error in the chat instead of nothing.
  - The Assistant's prompt: an action happened only if its tool result is in the conversation; approved-without-result, failed or declined means it did not, and it must say so.
  - MCP clients are not affected: they approve before calling, and the call runs and returns in one request.
### Verified
- `tsc -b` (0), `lint` (0), `vitest run` 1034/1034 (new: the persisted approval keeps its signature; the card says "didn't run" once settled and stays quiet mid-turn), `pnpm check` (0), e2e Assistant + board + MCP 19 passed.
...
- **A task could be put in a column that isn't on its board.** Luis asked the Assistant to move a task "to To do"; it landed in "Em progresso" and showed up under "On hold / Stuck". Traced in the local DB: the task's project had its own columns (forked by the MCP grader, so still carrying the old default names), the model picked one of them, and the "All tasks" board, not finding that column among the global ones, dropped the card into the **first** global column of the same category. Three causes, all fixed on the server so every client (the app, the MCP, the Assistant) gets the same answer:
  - `resolveStatus` only checked the column existed in the workspace. New `statusOnBoard` (`lib/task-statuses.ts`) accepts only a column of the task's own board (its project's own columns if forked, else the workspace's), on create, move (`PATCH /move`), edit (`PUT`) and archive's `moveTo`. A refusal lists the valid columns with ids and categories (`offBoardError`) so a model can pick again.
  - A task moved to another project (and its subtasks with it) lands in the new board's same-category column nearest its old position, instead of keeping a column the new board doesn't have.
  - The board places a card whose column isn't on screen by category **and position** (`nearestColumn`, `src/shared/task-columns.ts`, shared by server and client; tasks now carry `statusPosition`), not the first column of that category. Nothing reads a column's name: names are free text.
  - `move_task`/`list_task_statuses` and the Assistant's prompt say to fetch the columns with the task's `projectId`, match the person's words to a name or else choose by category and say which, and never use an unlisted column.
### Verified
- `tsc -b` (0), `lint` (0), `vitest run` 1028/1028 (new: `nearestColumn` picks by category and position on a board with no default names; through the MCP a move to another board's column is refused with the valid columns listed and the task unchanged, a create in another board's column is refused, a project change carries the task and its subtask to the matching column, an archive can't send tasks to another board), `pnpm check` (0), e2e task + MCP specs 40 passed.
...
- Verified: `tsc -b` (0), `lint` (0), `vitest run` 1018/1018 (the list cases now run on the original tools; new: a single-item call still works and a missing required field is refused; no twin is registered), `pnpm check` (0). Live on `tracking-local` (localhost): `list_tasks` returns the project's open tasks grouped Pendente → Em progresso, the undated one included.

## 2026-09-24 (91)
### Added
- **Batch tools: several items, one call, one approval.** Luis saw a request like "delete all my hours" ask for approval once per entry, in the app's Assistant and in MCP clients alike. Cause: every write tool call asks once (`mcp/chat-tools.ts` `needsApproval`; MCP clients do the same on their side), and the MCP only had one-item tools although the API already had `/bulk` routes. New: `create_tasks`, `move_tasks`, `update_tasks`, `delete_tasks`, `log_times` (each item exactly what the single tool takes, run in order by one shared helper, `mcp/batch.ts` `runBatch`, reporting every item's outcome; an error only when none went through), and `update_time_entries` / `delete_time_entries` on the existing all-or-nothing `/bulk` routes. The single and batch tools share one per-item function, so there's no second implementation. Both prompts (MCP server instructions, Assistant system prompt) now send several items to the batch tool, and say that one task for several people is one task with several `assigneeIds`.
- **"Approve all" in the Assistant**: when one answer asks for two or more actions, a bar above the cards approves or denies them all in one click (`ApprovalBatchBar`, `pendingApprovals.ts`); each card still shows what it does, and a batch card shows how many items it touches.
### Fixed
- **"My tasks" only listed today's tasks.** The Assistant's prompt mapped "my tasks" to `dueBy = today` (`lib/assistant-prompt.ts`), and `list_tasks`' own description pointed the same way, so a task due tomorrow or with no date never showed. Now "my tasks" with no day named is every open task (`assignee: me`, no `dueBy`); a date filter only when a day or period is named; "which are done" passes `includeDone` and reports the completed groups. `list_tasks` returns the tasks **grouped by status in the board's column order** (Backlog, Pendente, Em progresso, QA…), completed left out by default, so both surfaces answer the same way.
### Verified
- `tsc -b` (0), `lint` (0), `vitest run` 1017/1017 (new: batch create with a per-item failure report, all-failed is an error, batch move keeps the board's subtask cascade, batch delete, `log_times` + `delete_time_entries`, a read-only key sees no batch write tool, `list_tasks` includes future-dated and undated tasks and groups by column without completed ones; `ApprovalBatchBar` approves/denies all and stays hidden for one), `pnpm check` (0). Live MCP tests only against `tracking-local` (localhost), never the production server.
...
- **Pressing Enter several times on the subtask field created one subtask per press.** Reported by Luis from real use. Root cause, both sides: `QuickAddTask` clears the field only when the server answers (`onSuccess: reset`, deliberate so a failure never loses what was typed), so each Enter while the POST was in flight sent the same create again; and `POST /api/tasks` had nothing to recognise a repeat. Proven before the fix: the new route test against the old handler got `201` four times for one create.
  - Server: `CreateTaskSchema` takes an optional client `id` (UUID). `createTask` (the one path REST and MCP share) returns the task already made when that id exists in the workspace (`200`, no second notification or broadcast), refuses an id taken in another workspace (`409`, without echoing that task), and closes the race between two concurrent copies with `INSERT … ON CONFLICT(id) DO NOTHING`. Creates without an id work as before. The MCP `create_task` schema omits `id`: a model must never pick one.
  - Front: the form mints the id per capture (a new one only after a success), and a new shared `useSingleSubmit` hook drops a second submit while the first is in flight. Its lock is a ref, not the state, because two key presses can land before a re-render. Guarding per form rather than inside `useCreateTask` is deliberate: a mutation-level lock would drop legitimate parallel creates (timesheet cells, calendar drags).
  - Audit of every create flow: the same pattern was in **Projects → a project's task list** and **the board's new status column** (its button was disabled while pending, Enter was not); both now use `useSingleSubmit`. Saved reports, tags, favorites and comments clear or close synchronously, and the entry forms already block on `pending`, so a second press has nothing to send.
- **Text inputs with no side padding clipped their first letters** (the task quick-add and subtask fields). Measured: `padding-left: 0` on a pill (`rounded-full`) input, and Chrome clips an input's text to its rounded box. `Input`'s `bare` variant now owns a `px-2` gutter and a `rounded-md` corner (the same lesson `textarea-variants.ts` already recorded), the two screens stopped overriding it, and a new lint rule (`no-input-zero-padding`) refuses `px-0`/`pl-0`/`pr-0` on `<Input>`.
### Added
- **The subtask and quick-add lines have a real Add button**, always visible, with a spinner while the add is in flight, instead of Enter plus a decorative `↵`. The leading `+` icon was removed too, at Luis's request: next to a working Add it read as a second, broken add button.
- **"/" in a task description opens a block menu** (Text, Heading 1–3, Quote, Bulleted/Numbered list, Checklist), filtered as you type (pt-BR keywords, accents ignored). Block types are one catalogue (`editorBlocks.ts`) shared by the "/" menu and the selection bar. The "@" mentions and "/" now share one suggestion hook (`useSuggestionMenu`), so the popup and keyboard handling exist once.
- **The view button opens its menu on hover** (Modal / Sidebar, current one checked), as ClickUp's does, and on click for keyboard and touch.
### Changed
- **The task's fields lay out by the space they have**: a container query puts two per line in the modal and one per line in the sidebar.
- The editor's extensions are created with `useState(() => …)` instead of `useMemo(…, [])`, which removes three `eslint-disable` comments: the mentions' team now lives in the extension's own storage (`setMentionMembers`) instead of a ref captured at creation. `@tiptap/core` is now a direct dependency, since we augment its `Storage` type.
### Verified
- `tsc -b` (0), `lint` (0), `vitest run` 996/996 (new `tasks.idempotency.test.ts`: same id ×4 → one subtask; same name with different ids → two; foreign id → 409; non-UUID → 400; no id → 201. Plus `useSingleSubmit`'s effect in `QuickAddTaskInlineView.test.tsx` and the `bare` gutter in `input.test.tsx`), `pnpm check` (0). e2e (local): new `task-create-once.spec.ts` (five Enters against a server delayed by 1s → one subtask; the Add button creates) and three new cases in `task-view-mode.spec.ts` (hover menu, two-per-line vs one-per-line measured by position, "/" → checklist saved without the "/check" text); task specs 42 passed, 1 skipped. Screenshots checked: modal grid, the view menu, the "/" menu, the sidebar.

## 2026-09-24 (84)
### Added
- **A task opens in a two-column modal by default, with the old side panel one click away**, asked for by Luis, modeled on ClickUp's task view (its full-screen mode deliberately left out; the time-entry sheet is untouched). Content on the left, comments always visible on the right with the feed scrolling and the composer pinned (`TaskComments docked`); below `lg` the columns stack. A toolbar button swaps modal ↔ sidebar and the choice is persisted per browser (`uiStore.taskViewMode`, default `"modal"`, localStorage — decided over a per-account column so no migration was needed). A `/tasks/:id/comments` link opens the modal scrolled to the comments.
  - Componentized so neither mode duplicates a field: `TaskSheet.tsx` became the controller `TaskDetail.tsx` (every hook, mutation and autosave, each block built once) and two layout-only shells, `TaskModalShell` and `TaskSidebarShell`; the shared `TaskDetailToolbar` replaces `TaskSheetHeader`; `DescriptionField` moved to `TaskDescriptionField.tsx`. New `dialog` size `task` (viewport less a 24px margin, ClickUp's measured inset).
- **A formatting bar over selected text in a task's description** (`RichTextBubbleMenu`, tiptap's `BubbleMenu`): text/H1/H2/H3, bold, italic, underline, strikethrough, inline code, text color (the app's swatch palette via `ColorSwatchPicker`), link, bulleted/numbered list, checklist. Basic formatting only, no AI actions, at Luis's request. Kept tiptap rather than switching to Quill, which is what ClickUp runs: the stored descriptions, mentions, image upload and the MCP's plain-text reading all keep working with no data migration. New deps: `@tiptap/extension-text-style` (TextStyle + Color) and `@floating-ui/dom` (the menu's positioning peer). The bar's surface is a new `floating-toolbar-variants.ts` primitive.
### Changed
- **The description's type scale now matches ClickUp's**, measured from its task description's computed styles on 2026-09-24: 16px/1.5 body (was 18px), headings as multiples of it (H1 2×, H2 1.5×, H3 1.25×, weight 700, 22px above), code 0.875× mono, 2px block padding. A long description now expands when you click into it: a caret or selection inside the clipped box used to scroll its top out of view.
### Verified
- `tsc -b` (0), `lint` (0), `vitest run` 989/989 (new: plain-text reading of headings/colored text, `normalizeLinkHref`), `pnpm check` (0). e2e (local, `pnpm dev`): new `task-view-mode.spec.ts` (modal default with comments visible and no tab, switch to sidebar, survives a reload, back to modal; the selection bar applies H1 + red and both are saved), `task-comments` no longer clicks a Comments tab, `task-detail-panel`'s assignee assertion scoped to the assignee button (the now-visible activity feed also names the user); task specs 37 passed, 1 skipped. Screens checked by screenshot at 1440×900 light and dark, the sidebar, the bar with its color row, and 390px.
...
- **Deleting an account now deactivates it; no record is ever deleted.** Asked for by Luis ("excluir a conta não pode apagar dado nenhum"). Measured first on an in-memory SQLite with every migration: deleting a `user` row set the author of their time entries and tasks to NULL (so reports per person lost those hours), deleted their task assignments and recurring templates, and, when a legacy `workspaces.userId` pointed at them, cascaded away the whole workspace (every entry, project, task and comment of every member). Production has one workspace with `userId` NULL (read-only `SELECT`), so the cascade couldn't hit it today, but the self-service delete and the admin removal (which also purged solo-owned workspaces on purpose) both could.
  - `lib/account-deactivation.ts`: one batch bans the user (`banReason` "Account deactivated"), deletes their sessions and `member` rows, pauses their recurring templates and calendar auto-track (both crons would otherwise keep writing for someone who left), and turns off their digests. Refuses the only owner of a workspace (409 `LAST_OWNER`).
  - `POST /api/auth/delete-user` is served by `routes/account.ts` (Better Auth's `deleteUser` is now disabled); `/api/auth/admin/remove-user` answers 403; the admin `DELETE /api/admin/users/:id` deactivates, checking the admin role before it answers anything that names a workspace.
  - Being invited again (`sendInvitationEmail`) lifts a deactivation and never a moderation ban.
  - Settings → Danger zone is "Deactivate account" (no password field: the fresh-session gate is the re-authentication); the admin page's Remove says what actually happens.
### Fixed
- **"Sign out other devices" and revoking one session failed with 403 for anyone signed in over a day** — the same wrong premise as (80): `requireFreshSession` was put on the three `revoke-*session*` routes as if Better Auth had gated them, but Better Auth only re-reads the session from D1 there (`sensitiveSessionMiddleware`). The gate now stays only on `unlink-account` and `delete-user`, where Better Auth had it.
- **A refused sensitive action said only "Failed to …".** The gate's 403 carried `error` but not `message`, which the auth client displays; it now carries both, so the screen says "Please sign in again before changing these account settings."
- Verified locally only, with accounts created by the tests: `vitest run` 985/985 (new `account-deactivation.test.ts`, in-memory: every record and its author kept, lockout, sole-owner refusal, invitation lifts a deactivation but not a moderation ban); `pnpm check` (0); `lint` (0). New e2e `account-deactivation.spec.ts`: a member with a day-old session signs out their other device (200, the other session is gone from D1), is asked to sign in again before deactivating, deactivates, can then neither sign in nor sign up with the same email, their hour stays credited to them, and after a new invitation they sign in and accept. The only owner is refused and Better Auth's hard delete answers 403. `account-rename-stale-session.spec.ts` now expects revoke to work and unlink to stay gated with a readable message. Screens checked: the danger-zone dialog, and a day-old session's toasts ("Signed out all other devices"; "Please sign in again…"). Full e2e: 133 passed, 1 skipped, 5 failed. 4 of the failures are older than this change and fail the same way on the clean tree (the code in production): `invite-only` "login page offers no self sign-up" and "invitee without a workspace…", `session-security` S-01, and `workspace-member-removal` "removed by the owner…". The fifth was this spec's `revoke-other-sessions` getting 500 from `SQLITE_BUSY` (the test's `wrangler d1 execute` writing the dev server's SQLite file from a second process under two workers); it passes on its own and in parallel, and the two specs that age a session now retry once. `e2e/local-d1.ts` holds that helper.
...
### Changed
- **Tasks rail: each client folds its projects away** with a chevron in the row's right slot; which clients are folded persists in `uiStore.collapsedRailClients`. The ghost button's open-menu fill on `aria-expanded` is neutralised there, since it's a disclosure, not a menu. The fold animates: the projects sit in the shared `Collapsible` primitive (height keyframes over `duration-base`, `ease-out-quart`) and the chevron turns on the same duration — the first cut unmounted them, which Luis found too abrupt. Measured in the browser: height steps 68→48→32→…→0 closing and 0→20→36→…→68 opening across ~200ms, where it had jumped straight between the two.
...
- **Self-service password reset, and account creation is now open (invite-gated) instead of dev-only.** Three things landed together, requested by Luis after asking for the reset-password link and finding there was no sign-up entry point either:
  - **Reset password** (`/reset-password`, linked from the login card): Better Auth's `sendResetPassword`/`requestPasswordReset`/`resetPassword` wired to a new email template; resetting revokes the account's other sessions (`revokeSessionsOnPasswordReset`).
  - **Invite emails are now a magic-link sign-in, not a plain URL** — clicking one authenticates *and* lands on `/accept-invite` already signed in, instead of a separate login step. `sendInvitationEmail` calls `auth.api.signInMagicLink` with invite metadata; the shared `sendMagicLink` callback routes to the invitation template (with the magic URL) when that metadata is present, the plain sign-in template otherwise — same infra, no new token/verification code. After accepting, an optional "set a password" step (new `POST /api/settings/set-password`, wrapping Better Auth's server-only `setPassword`) lets the person add one so Google/passwordless/password all work later; skippable.
  - **`/sign-up` (password account creation) is enabled in production**, gated by the existing invite-only check (`databaseHooks.user.create.before`, unchanged — covers every creation path uniformly). To close the race where a stranger could squat someone else's invited-but-unclaimed email with a password of their own choosing, production now requires email verification before a password sign-up gets a session (`requireEmailVerification: true` + a new "verify your email" template) — mirrors the guarantee `requireEmailVerificationOnInvitation` already gave invite acceptance. Dev/e2e are untouched (`!import.meta.env.DEV`), so `e2e/auth.ts`'s immediate-sign-in-after-signup keeps working.
  - New shared `PasswordInput` (`components/ui/password-input.tsx`, wraps `Input` with a show/hide toggle) used on every password field across login, sign-up, reset-password, and the invite's set-password step, instead of a bare `type="password"`.
  - Verified end-to-end against local dev via curl (not a full test suite, called out here on purpose): reset-password token creation → callback redirect → reset → new password works, old one 401s; invite → magic-link click auto-authenticates (`emailVerified: true`, session cookie) → accept → set-password; sign-up refuses a non-invited, non-`@example.com` address with `INVITE_ONLY`. `tsc -b` (0), `lint` (0), `pnpm check` (0), `vitest run` (978/978, no regressions — no new automated tests added for these flows, a gap worth closing next).
### Deployed
